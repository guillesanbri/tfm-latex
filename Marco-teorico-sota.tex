...
Para diseñar esta red, se extiende el trabajo de Tan et al. \cite{}, y se emplea \textit{Neural Architecture Search - NAS} (una técnica que automatiza la búsqueda y configuración de arquitecturas de redes neuronales) buscando optimizar el \textit{accuracy} \todo{En español exactitud me suena feo y precisión me suena a precision} y el número de operaciones en coma flotante (FLOPS). Como resultado, se obtiene una arquitectura que emplea principalmente bloques de capas MBConv (\textit{Mobile Inverted Bottleneck Residual Convolution}) \cite{mobilenetv2} y \textit{Squeeze-and-Excitation} \cite{squeeze and excitation}.
